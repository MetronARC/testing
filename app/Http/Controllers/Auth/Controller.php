  ...
  public function __construct()
  {
    $this->middleware(function ($request, $next) {
      ...
      if (!empty($auth_data) && $request->segment(1) == 'login') {
        if (in_array($auth_data['role'], ['member_basic', 'member_premium'])) {
        } elseif (in_array($auth_data['role'], ['vendor'])) {
        } elseif (in_array($auth_data['role'], ['superadmin', 'admin'])) {
          return redirect(env('DOMAIN_ADMIN_URL'));
        }
      } elseif (empty($auth_data) && $request->segment(1) == 'register') {
