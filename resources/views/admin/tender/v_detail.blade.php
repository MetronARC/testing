  ...
  <div class="row">
    <div class="col-lg-12">
      <div class="main__header d-flex justify-content-between">
        <h1 class="h3"><strong>Tender</strong> Detail</h1>
          ...
          <div class="card-body">
            <form action="{{ url()->current() }}" method="post" onsubmit="submitForm(this, event)">
              ...
              <div class="row">
                <div class="col-lg-6">
                  <div class="mb-3">
                    <label for="tender__tender_number" class="form-label">Tender Number</label>
                    <input type="text" class="form-control" id="tender__tender_number"
                      placeholder="Input Tender Number" value="{{ $tender['tender_number'] }}" readonly disabled>
                      ...
                      @if (!empty($shipyards))
                        @foreach ($shipyards as $_val)
                          <option value="{{ $_val['id'] }}"
                            {{ $_val['id'] == $tender['shipyard_id'] ? 'selected' : '' }}>{{ strtoupper($_val['name']) }}
                          </option>
                        @endforeach
                      @endif
                    </select>
                ...
                <div class="col-lg-6">
                  <div class="mb-3">
                    ...
                    <input type="text" class="form-control" name="phone" id="tender__phone"
                      placeholder="Input Whatsapp Number" value="{{ $tender['phone'] }}">
                    <div class="invalid-feedback"></div>
                  </div>
                </div>
                <div class="col-lg-6">
                ...
                <div class="col-lg-6">
                  <div class="mb-3">
                    ...
                    <select class="form-select select2-input" name="tender_status" id="tender__status">
                      @foreach (config('custom')['tender_status'] as $_key => $_val)
                        <option value="{{ $_key }}" {{ $tender['tender_status'] == $_key ? 'selected' : '' }}>
                          {{ strtoupper($_val) }}
                        </option>
                      @endforeach
                    </select>
                ...
                <div class="col-lg-6">
                  <div class="mb-3">
                    ...
                    <input type="text" class="form-control" name="closed_date" id="tender__closed_date"
                      placeholder="Input Closed Date"
                      value="{{ $tender['closed_date'] ? date('m/d/Y', strtotime($tender['closed_date'])) : null }}">
                    <div class="invalid-feedback"></div>
                  </div>
                </div>
              </div>
                ...
                <div class="col-lg-12">
                  <div class="mb-3">
                    ...
                    <textarea type="text" class="form-control" name="description" id="tender__description"
                      placeholder="Input Description">{!! $tender['description'] !!}</textarea>
                    <div class="invalid-feedback"></div>
                  </div>
                </div>
              </div>
